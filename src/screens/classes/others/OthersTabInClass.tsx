...
const OthersTabInClass = (args: { classId: string }) => {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <TouchableOpacity
        style={styles.item}
        onPress={() => navigation.navigate(SurveyNavigationName.SurveyList as any, { classId })}
      >
      ...
      {auth?.user?.role === Roles.STUDENT && (
        <TouchableOpacity
          style={styles.item}
          onPress={() =>
            navigation.navigate(AbsenceNavigationName.RequestAbsence as any, { classId })
          }
        >
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (auth?.user?.role === Roles.LECTURER) {
            navigation.navigate(AttendanceNavigationName.AttendanceMain, { classId });
          } else {
            navigation.navigate(AttendanceNavigationName.GetAttendanceRecord, { classId });
          }
        }}
      >
      ...
      {auth?.user?.role === Roles.LECTURER && (
        <TouchableOpacity
          style={styles.item}
          onPress={() => navigation.navigate(AbsenceNavigationName.AbsenceList, { classId })}
        >
